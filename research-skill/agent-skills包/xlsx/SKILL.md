---
name: xlsx
description: "综合电子表格创建、编辑和分析，支持公式、格式、数据分析和可视化。当 Claude 需要处理电子表格（.xlsx、.xlsm、.csv、.tsv 等）时使用：(1) 创建具有公式和格式的新电子表格，(2) 读取或分析数据，(3) 修改现有电子表格同时保留公式，(4) 电子表格中的数据分析和可视化，或 (5) 重新计算公式"
...
### 零公式错误
- 每个 Excel 模型必须交付零公式错误（#REF!、#DIV/0!、#VALUE!、#N/A、#NAME?）

### 保留现有模板（更新模板时）
- 研究并完全匹配现有格式、样式和约定修改文件
- 永远不要对具有既定模式的文件强加标准化格式
- 现有模板约定始终覆盖这些指南
...
#### 硬编码的文档要求
- 在旁边的单元格中注释或输入（如果在表格末尾）。格式："来源：[系统/文档]，[日期]，[特定参考]，[URL（如适用）]"
- 示例：
  - "来源：公司 10-K，2024 财年，第 45 页，收入说明，[SEC EDGAR URL]"
  - "来源：公司 10-Q，2025 年第二季度，展览 99.1，[SEC EDGAR URL]"
  - "来源：Bloomberg Terminal，2025 年 8 月 15 日，AAPL US 股票"
...
**LibreOffice 需要用于公式重新计算**：您可以假设已安装 LibreOffice，用于使用 `recalc.py` 脚本重新计算公式值。该脚本在首次运行时自动配置 LibreOffice
